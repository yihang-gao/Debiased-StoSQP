...
We propose a stochastic sequential quadratic programming algorithm with averaged gradients for debiasing (Debiased-StoSQP), where we decide the search direction by performing a quadratic expansion of the objective with a linearization of the constraints.
...
Our algorithm achieves global almost sure convergence in terms of first-order optimality conditions (i.e., KKT conditions), and it exhibits local asymptotic normality, where the limiting covariance matrix represented by the Fisher information matrix is optimal in the H\'ajek and Le Cam's sense. 
Additionally, a plug-in estimator of the covariance matrix is provided for practical statistical analysis. 
Through extensive experiments on benchmark problems in the CUTEst library, constrained linear and logistic regression problems, portfolio allocation problems, and constrained generalized linear models (e.g., Poisson regression), with both synthetic and real data, we demonstrate the performance of our algorithm.
\end{abstract}
...
%% REDUNDANT WITH NEXT PAR %% Such constrained stochastic optimization problems have wide applications in statistics, machine learning, and optimization. 
...
Stochastic optimization algorithms for optimizing an objective $f(\bm{x})$ have a rich history and can be traced back (at least) to stochastic gradient descent (SGD), which solves (\ref{original_problem0}) in an unconstrained setting. 
...
\michael{I reworded.  Is that okay?}
\end{itemize}
...
To accomplish this, we introduce a novel stochastic Sequential Quadratic Programming algorithm (\textcolor{blue}{Debiased-StoSQP})
to solve Problem (\ref{original_problem0}), with global almost sure convergence guarantees.
...
Importantly, the presence of inequality constraints in Problem (\ref{original_problem0}) introduces a bias in the solutions of the quadratic subproblems for direction estimates. 
...
\michael{Why do we have this par in the intro?  I think that the only place we refer to it is in the empirical eval section, so why not put it there, since it doesn't seem to add so much value here in the intro?}
...
\begin{enumerate}
    \item[\textbf{(a)}] We revisit a standard SQP algorithm (namely, RelaxedSQP), which is applicable to deterministic objectives in the form of Problem~\eqref{original_problem0}, where a relaxation parameter is introduced for the feasibility of the quadratic subproblem. Significantly, we establish a connection between this relaxation scheme and constraint qualifications, providing a deeper understanding of constrained optimization problems.
\michael{Explicitly point to the algorithm below.}
    

    \item[\textbf{(b)}] Building on the RelaxedSQP, we introduce Debiased-StoSQP, a stochastic counterpart of RelaxedSQP.
...
We also employ statistical inference techniques to gain insights into the locally asymptotic behavior of our algorithm. 
...
Note that, unlike SGD, SQP is a constrained Newton's method, where we calculate the noisy Hessian matrix $\nabla^2 f(\bm{x};\zeta)$ in each iteration. Therefore, the plug-in estimator for the covariance matrix does not significantly increase the computational complexity. Compared with existing works studying the asymptotic behavior of algorithms, we derived the asymptotic normality for the algorithm with averaged gradients. 
...
Previous works typically rely on the independence of gradients, while we consider averaged gradients that are highly correlated.
...
While there exists an extensive body of work focusing on the statistical properties of SGD and its various adaptations \cite{polyak1992acceleration, chen2020statistical}, the local statistical behavior of stochastic Newton's methods remains relatively unexplored. 
...
Section~\ref{sec:relaxed_stochSQP} is devoted to the introduction of our proposed relaxed stochastic SQP method, 
\michael{This is an example of a place where we shuld put in an explicit reference to the Algorithm numver and/or name. Ditto in the next sentences, and elsewhere.}
...
Throughout the paper, we provide sketches of proofs following the theorems to aid in comprehension, but we defer all detailed proofs to the Appendix.
\michael{We have multiple appendices, no?}
...
At the current iterate $\bm{x}_k$, the general SQP algorithm 
...
The solution $\bm{p}_k$ of Subproblem~\eqref{original_sqp_subproblem} then serves as the direction for updating the variable $\bm{x}$ in Problem~\eqref{original_det_problem}, i.e., $\bm{x}_{k+1} = \bm{x}_{k} + \alpha_k \bm{p}_k$, where $\alpha_k >0$ is the step size.
...
As indicated by Proposition \ref{prop_thetak_to_0}, 
...
More specifically, throughout the paper, instead of solving Subproblem~\eqref{original_sqp_subproblem} for the search direction in each iteration, we alternatively focus on the following relaxed SQP subproblem:
\begin{equation}
...
\michael{What does ``substantiated by existing literature'' mean? Just that they assume it too?}
...
A detailed exposition of this can be found in Appendix \ref{sec:appendix1.2}.


Before delving into the properties of the algorithm, 
\michael{Point explicitly to which algorithm, by name and/or number, everywhere consistently; otherwise, it can be hard to follow.}
it is imperative to articulate the Karush-Kuhn-Tucker (KKT) optimality conditions, as well as the associated KKT residual specific to Problem~\eqref{original_problem0}. 
    The KKT condition and  the corresponding KKT residual for Problem (\ref{original_problem0}) at $\bm{x}$ is formalized as
    \begin{equation}
    \label{kkt_condition}
        \begin{split}
...
\end{equation}
Furthermore, if we let $(\bm{\lambda}_k^{\text{sub}}, \bm{\mu}_{1,k}^{\text{sub}},\bm{\mu}_{2,k}^{\text{sub}})$ be the Lagrangian multipliers of the relaxed SQP Subproblem (\ref{relaxed_sqp_subproblem}) at $\bm{x}_k$, then 
\begin{equation}
\label{eq5}
...
Detailed proofs are included in Appendix~\ref{sec:appendix1.4}.
...
In this section, we developed a stochastic optimization algorithm with almost sure convergence guarantees (Debiased-StoSQP) for solving Problem (\ref{original_problem0}).  
...
Subsequently, we extend these results to achieve the almost sure "$\lim$" convergence by incorporating the least squares estimates of dual variables. Before analyzing the convergence performances of the proposed 
\michael{don't say ``proposed'' give an explicit reference}
algorithm, we describe the algorithm the following steps:
\begin{itemize}
    ...
    \end{split}
    \end{equation*}
    and verify that $\bm{p} = \bm{0}$ is the solution. Projected gradient descent and projected Newton's methods are popular and efficient solvers for the box-constrained quadratic problem. If the algorithm terminates with $\theta_k < \Tilde{\theta}$, where the small $\Tilde{\theta} \in (0,1]$ is the tolerance included in our 
\michael{don't say ``our'' give an explicit reference}
algorithm, we have reasons to doubt 
\michael{Huh? Clarify that.}
that the iterate $\bm{x}_k$ approaches an undesirable point, where EGMFCQ does not hold. 
...
Here, the direction $\Bar{\bm{p}}_k$ is ``descent'' for the merit function, owing to the convergence of  $\Bar{\bm{g}}_k$ to $\nabla f(\bm{x}_k)$ and the positive-definiteness of $\bm{B}_k$. 
    ...
    An update scheme for these dual variables, which is crucial for examining the local convergence properties of the iterates, is provided in (\ref{update_dual}) 
    \michael{when we refer to things, like there, but elsewhere, can we refer to it ans an equation or lemma or problem or subproblem or whatever it is, just to help orient the reader}
...
\end{split}
\end{equation}
The estimated optimal Lagrangian multipliers   $(\bm{\lambda}_{k}^{*}, \bm{\mu}_{1,k}^{*}, \bm{\mu}_{2,k}^{*})$ corresponding to $\bm{x}_k$ serve as one of the feasible solutions of Problem (\ref{est_opt_lagrangian}) evaluated at $\bm{x}_k$. The detailed proofs can be found in Appendix \ref{sec:appendix2}.
...
In this section, we further elaborate on Algorithm \ref{alg_stoch_relaxed_sqp_averaging_grad} and show the asymptotic normality property, where the proposed algorithm is asymptotically optimal in H\'ajek and Le Cam's sense, as shown in (\ref{M-estimator}). 
\michael{Clarify if \ref{M-estimator} is an equation or section or lemma or whatever.}
\michael{No to the use of ``the proposed algorithm'' and ``the elaborated algorithm'' and so on.}
The elaborated algorithm is shown in Algorithm \ref{alg_stoch_relaxed_sqp_averaging_grad_Bk}, where we provide detailed update scheme for approximate Hessian matrix $\bm{B}_k$ (as shown in \textbf{Step} $\mathbf{2^{\prime}}$) and dual variables (as shown in \textbf{Step 6}).
...
Consequently, the almost sure convergence results established for the latter are also applicable to the former. 
\michael{Don't refer to former and latter, refer to each explicitly.}
...
\begin{algorithm}[htb]
 \caption{(Optimal) Debiased-StoSQP \michael{We don't use this name. We should have different names for this and the previous alg and refer to each as such.}}
...
the KKT condition and the strong convexity of the SQP subproblem further imply that $\bm{p}_k$ and $\Bar{\bm{p}}_k$ are the solution of the following equality-constrained problems, respectively:
   \begin{equation}
    ...
    \end{split}    
   \end{equation}
   Without the loss of generality, we assume that the relaxation parameter is unit according to Lemma \ref{theorem_local_relaxation_parameter}. The LICQ condition at $\bm{x}^{*}$ also implies the LICQ at $\bm{x}_k$ when $\bm{x}_k$ is sufficiently close to $\bm{x}^{*}$. Then the KKT system of Problem (\ref{eq9}) shows that
   \begin{equation}
   \footnotesize
\label{eq29}
    \left( \begin{array}{c}
...
The result (\ref{M-estimator}) 
\michael{be explicit about what that ref is, here and elsew here, incl below}
...
Our result in Corollary \ref{coro_optimality} complements this by demonstrating that the optimal local convergence behavior of the primal variable is achieved, as characterized by the covariance matrix $\bm{\Omega}^{*}$. Here, considering only the primal variable $\bm{x}_k$, our results match (\ref{M-estimator}) by applying knowledge of linear algebra \cite[Corollary~2.3]{tian2009inverse}.
...
In this section, we describe comprehensive experiments we have conducted to demonstrate the effectiveness of the \textcolor{blue}{Debiased-StoSQP algorithm (Algorithm \ref{alg_stoch_relaxed_sqp_averaging_grad_Bk})}. 
...
\michael{Why not put those examples here, that seems like a better use of space in the intro.}
...
\end{itemize}
We first conduct a comparison between the proposed Debiased-StoSQP (Algorithm \ref{alg_stoch_relaxed_sqp_averaging_grad_Bk}), ActiveSet-SQP \cite{na2023inequality} and StoSQP \cite{curtis2023sequential}, where we evaluate each method by the KKT residual (\ref{kkt_condition}) and feasibility error. 
...
\makebox[\linewidth][c]{%
  ...
  [KKT residuals ] 
  {
    ...
    \caption{		
	 KKT residuals and feasibility errors of Debiased-StoSQP, StoSQP, and ActiveSet-SQP on CUTEst problems.}	
...
Here, we implement our algorithm (Algorithm \ref{alg_stoch_relaxed_sqp_averaging_grad_Bk}) 
\michael{give it its name also}
...
The exact mean, covariance, and expectations are inaccessible in practice. Instead, we estimate the stochastic gradient and Hessian of the expected objective using available observations, and we apply our algorithm to solve the problems. For the portfolio strategy $\bm{x}$, we use historical data from the past year as training samples. 
...
\begin{figure}[ht]
  ...
  \subfloat[Stock 1, LOG] {
    ...
    \caption{		
             Weights and returns. \michael{More detailed caption, so that it is self-contained.}
            }	
    \label{fig:portfolio}
\end{figure}
...
Similarly, employing the constrained Poisson model and solving it via our algorithm, 
...
\section{Conclusion}
In this work, we proposed \textcolor{blue}{Debiased-StoSQP}, 
\michael{Which one?}
...
Furthermore, under certain mild conditions, we developed the asymptotic normality for our proposed algorithm.
This is a particularly surprising and novel result since the gradients in our algorithm are highly correlated, in contrast with previous work that primarily relies on the independence of gradients. 
...
The asymptotic normality behavior still holds for the algorithm 
\michael{which one}
with sketching. 
