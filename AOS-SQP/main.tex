...
\michael{I'm confused. What does it mean that the algorithm ``performs a quadratic expansion of the objective with a linearization of the constraints''?}
\michael{Also, this is still an awkward name. Can we get something that is easier to say. It sounds silly, but if it is easier to say, more people will say and remember it.}
A key challenge for constrained stochastic optimization is the biased search direction, 
\michael{What about that direction? Finding it?}
even though \textcolor{blue}{we can access an unbiased estimate of stochastic gradients.} 
To address this, we introduce averaging gradients in the proposed algorithm to mitigate the biasedness. 
...
\textcolor{blue}{We introduce two different step sizes with different decay rates for updating the iterates $\bm{x}_k$ and the gradient, respectively, which is a key idea. 
...
This ensures that the algorithm is driven by the most current and relevant gradients, contributing to its effective performance.
\michael{This par is almost word for word redundant with above.  Include it here or there but not both.}
...
\textcolor{blue}{Now, we are ready to deal with standard SQP subproblems (\ref{original_sqp_subproblem}), which are probably infeasible as discussed, by including a relaxation parameter $\theta_k \in (0,1]$ in each iteration.
...
Here, the direction $\Bar{\bm{p}}_k$ is ``descent'' for the merit function, owing to the convergence of  $\Bar{\bm{g}}_k$ to $\nabla f(\bm{x}_k)$ and the positive-definiteness of $\bm{B}_k$. 
    ...
    An update scheme for these dual variables, which is crucial for examining the local convergence properties of the iterates, \textcolor{blue}{is provided in (\ref{update_dual}) in Section \ref{sec:asymptotic}}.
\end{itemize}
...
\textcolor{blue}{This theorem is an intermediate result that the KKT residual of the iterates obtained by the proposed AvgStoSQP algorithm achieves the ``$\lim \inf$'' convergence. The detailed proofs are available in Appendix \ref{sec:appendix2}.}
...
In contrast, StoSQP lacks this beneficial property, and it suffers from oscillations brought by the stochastic gradient. 
The ActiveSetSQP, which uses a stochastic line search method, necessitates an increasing sample size, and it employs a safeguard technique to ensure the accuracy of the line search. 
...
However, when the noise level is relatively low, our algorithm can effectively mitigate the noise through averaging gradient, and it can achieve similar and even better performances than ActiveSetSQP. 
