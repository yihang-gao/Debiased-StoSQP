...
We propose a stochastic sequential quadratic programming algorithm \textcolor{blue}{with averaged gradients for debiasing (Debiased StoSQP), 
where we decide the search direction by performing a quadratic expansion of the objective with a linearization of the constraints.}
...
\yihang{I mean the algorithm finds the search direction from the quadratic expansion of the objective and linearization of constraints.}
...
\yihang{see the first sentence in blue. we mention that we decide the search direction by...}
even though we can access an unbiased estimate of stochastic gradients.
To address this, we introduce averaging gradients in the proposed algorithm to mitigate the biasedness (debiasing). 
