...
We propose a relaxed stochastic sequential quadratic programming (R-StoSQP) algorithm, 
...
A key challenge for constrained stochastic optimization is the biased search direction, even though the stochastic gradient estimation remains unbiased. 
To address this, we introduce averaging gradients in the proposed algorithm for debiasing. 
...
\end{example}
\michael{Clarify what is that example an example of.}
...
Asymptotic analysis serves as a critical tool for a nuanced understanding of the local behavior of iterates in stochastic algorithms. In the context of constrained optimization, we let the primal-dual solution $(\bm{x}^{*}, \bm{\lambda}^{*}, \bm{\mu}_1^{*}, \bm{\mu}_2^{*})$, especially the primal solution $\bm{x}^{*}$, as the optimal solution of the problem (\ref{original_problem0}) with expected objective. 
\michael{What is that sentence saying.}
Here, the dual variable $(\bm{\lambda}^{*}, \bm{\mu}_1^{*}, \bm{\mu}_2^{*})$ corresponds to the equality constraints $\bm{c}(\bm{x}_k) = \bm{0}$, the lower-bound box constraints $\bm{\ell} - \bm{x} \leq \bm{0}$, and the upper-bound box constraints  $\bm{x} - \bm{u} \leq \bm{0}$, respectively. 
...
\textit{Can we develop an (asymptotically) optimal algorithm, in H\'ajek and Le Cam's sense, for Problem \eqref{original_problem0}, in a manner analogous to the the classical M-estimator for the unconstrained problem?  }
}
\end{itemize}
...
To accomplish this, we introduce a novel stochastic Sequential Quadratic Programming (namely, R-StoSQP) algorithm, 
...
\begin{enumerate}
    ...
    Here, we achieve the asymptotically optimal normality in terms of the covariance matrix, according to the min-max lower bound by Duchi and Ruan. 
...
We introduce two different step sizes for updating the iterates $\bm{x}_k$ and the gradient, respectively. 
...
Using recent advancements in martingale difference arrays, our asymptotic analysis is comprehensive for dealing with the algorithm with averaging gradients where the gradients are highly correlated. 
...
\begin{definition}
    ...
    \michael{Those last two sentences are presumably not part of the definition and should be a remark after.}
\end{definition}
...
\michael{In this section, we do such-and-such.  To accomplish this, }
...
In the next part, 
\michael{What does ``next part'' refer to?}
...
Theorem \ref{theorem_minimax_optimality} generalized from Duchi and Ruan \cite[Theorem~1]{duchi2021asymptotic} established a lower bound for stochastic algorithms in solving \eqref{original_problem0}. 
\michael{Broken ref.}
Our results in Corollary \ref{coro_optimality} complement this by demonstrating that the optimal local convergence behavior of the primal variable is achieved, as characterized by the covariance matrix $\bm{\Omega}^{*}$.
...
\begin{equation*}
    ...
    \end{array}\right).
\end{equation*}
The provided practical estimator for the covariance matrix can be used as a surrogate to the exact matrix for analyzing the local behavior of algorithms and conducting statistical inference.
...
We have conducted comprehensive experiments to demonstrate the effectiveness of Relaxed StochSQP. 
...
Implementation details are made available as our supplementary code, which can be accessed at [XXX].
\michael{Broken ref.}
...
We randomly select some subset of constrained optimization problems from the library and artificially include noise to gradient and Hessian as follows:
\michael{Can we do this better than randomly, maybe choose a few to illustrate things that are challending for us.}
\begin{itemize}
...
\end{itemize}
We first conduct a comparison between the proposed Relaxed StochSQP, ActiveSet SQP \cite{na2023inequality} and StochSQP \cite{curtis2023sequential}, evaluated by the KKT residual (\ref{kkt_condition}) and feasibility error. 
For each algorithm and problem, we run $10^5$ iterations. 
Our empirical results indicate that the Relaxed StochSQP algorithm consistently outperforms the StochSQP, both of which adopt fully stochastic gradients and Hessian. 
...
In contrast, StochSQP lacks this beneficial property and suffers from oscillations brought by the stochastic gradient. 
The ActiveSet SQP, which uses a stochastic line search method, necessitates an increasing sample size and employs a safeguard technique to ensure the accuracy of the line search. 
...
However, when the noise level is relatively low, our algorithm can effectively mitigate the noise through averaging gradient, and achieve similar and even better performances than ActiveSet SQP. 
...
using the estimators and limiting normality results in Theorem \ref{theorem_pratical_normality}. The performance of the method in terms of asymptotic normality is measured by the coverage rate (Cov Rate) of the confidence intervals and their average length (Avg Len) over 200 runs. 
...
\section{Conclusion}
In this work, we proposed a fully stochastic Newton's method for solving constrained optimization problems, called Relaxed StochSQP. 
...
With our results, we are capable of applying Relaxed SQP to perform online inference for constrained optimization problems, as we have demonstrated with our empirical results. 
