...
def solve_relax_stoch_SQP(prob, prop_prob, variables, hyper):
    ...
    for j in range(hyper.max_iter):
        ...
        err, v = feasibility_subprob(prob, prop_prob, variables, hyper)
        if err:
            return err
        ...
        err, d_xyz, d_dual_eq, d_dual_bound = solve_sqp_subprob(B, v, prob, prop_prob, variables, hyper)
        if err:
            return err

        # pk = d_xyz[0:prop_prob.dim_n]
        # step size calculation and parameters/vars update
        err, step_size = get_step_size(d_xyz, B, prob, prop_prob, variables, hyper)
        if err:
            return err
