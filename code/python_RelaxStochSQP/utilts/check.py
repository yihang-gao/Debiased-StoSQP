...
class PropertyProblem:
    def __init__(self, prob, seed):
        ...
        self.new_dim = self.dim_n + self.mcl + self.mcu

        # lower and upper bounds for (x, y, z)
        if self.mcl > 0:
            self.yl = np.array([-1e15] * self.mcl)
            self.yu = np.array([0.0] * self.mcl)
        else:
            self.yl = []
            self.yu = []
        if self.mcu > 0:
            self.zl = np.array([0.0] * self.mcu)
            self.zu = np.array([1e15] * self.mcu)
        else:
            self.zl = []
            self.zu = []

        self.xyzl = np.append(self.xl, np.append(self.yl, self.zl))
        self.xyzu = np.append(self.xu, np.append(self.yu, self.zu))
